...
// This is the Vrekrer_scpi_parser library collapsed into 1 header file, with my adaptations:
// extra parameter in ProcessInput and GetMessage: ignore_char
// removed the timeout in GetMessage
// more stuff in PrintDebugInfo
...
/*!
  Main class of the HB_SCPI_Parser library.
*/
class SCPI_Parser {
 public:
  //Constructor
  ...
  void ProcessInput(Stream &interface, const char* term_chars, char ignore_char);
  //Gets a message from a Stream interface
  char* GetMessage(Stream& interface, const char* term_chars, char ignore_char);
...
// ****************************************************************************************
// ****************************************************************************************
// This is where the .h file ends and the .cpp file starts
// ****************************************************************************************
// ****************************************************************************************
// ****************************************************************************************
// ****************************************************************************************
...
/*!
 ...
 @see Execute
*/
void SCPI_Parser::ProcessInput(Stream& interface, const char* term_chars, char ignore_char) {
  char* message = this->GetMessage(interface, term_chars, ignore_char);
  if (message != NULL) {
...
/*!
 ...
 @param ignore_char Character to ignore. 0 if none. e.g. '\r'
...
*/
char* SCPI_Parser::GetMessage(Stream& interface, const char* term_chars, char ignore_char) {
  while (interface.available()) {
    // Read the new char
    char ch = interface.read();
    if ((ignore_char != 0) && (ch == ignore_char))
        continue; 
...
///Prints registered tokens and command hashes to the serial interface
void SCPI_Parser::PrintDebugInfo(void) {

  Serial.println(F("*** DEBUG INFO ***"));
  Serial.println();
  if (tokens_size_ < SCPI_MAX_TOKENS) {
    Serial.printf("TOKENS: %d/%d.\n",tokens_size_,SCPI_MAX_TOKENS);
  } else {
    Serial.print("TOKENS: table full or overloaded.\n");
  }
  if (codes_size_ < SCPI_MAX_TOKENS) {
    Serial.printf("COMMANDS: %d/%d.\n",codes_size_,SCPI_MAX_COMMANDS);
  } else {
    Serial.print("COMMANDS: table full or overloaded.\n");
  }
  Serial.printf(F("TOKENS :"));
